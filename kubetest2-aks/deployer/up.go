...
var (
	ccm       testComponent = "ccm"
	azuredisk testComponent = "azuredisk"
	azurefile testComponent = "azurefile"
	unknown   testComponent = "unknown"
)
...
func (d *deployer) prepareCustomConfig() ([]byte, error) {
	...
	if testingComponent == ccm {
		imageMap["{CUSTOM_CCM_IMAGE}"] = fmt.Sprintf("%s/azure-cloud-controller-manager:%s", imageRegistry, d.CCMImageTag)
		imageMap["{CUSTOM_CNM_IMAGE}"] = fmt.Sprintf("%s/azure-cloud-node-manager:%s-linux-amd64", imageRegistry, d.CCMImageTag)
	} else if testingComponent == azuredisk {
		imageMap["{CUSTOM_AZURE_DISK_IMAGE}"] = fmt.Sprintf("%s/%s/azuredisk-csi:%s", registryURL, registryRepo, d.AzureDiskImageTag)
	} else if testingComponent == azurefile {
		imageMap["{CUSTOM_AZURE_FILE_IMAGE}"] = fmt.Sprintf("%s/%s/azurefile-csi:%s", registryURL, registryRepo, d.AzureFileImageTag)
	}
...
func (d *deployer) verifyUpFlags() error {
	...
	if d.CCMImageTag != "" {
		testingComponent = ccm
	}
	if d.AzureDiskImageTag != "" {
		testingComponent = azuredisk
	}
	if d.AzureFileImageTag != "" {
		testingComponent = azurefile
	}
	if testingComponent == unknown {
		return fmt.Errorf("testing component is unset")
	}
	return nil
}
