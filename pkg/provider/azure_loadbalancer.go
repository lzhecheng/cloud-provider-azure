...
func (az *Cloud) cleanOrphanedLoadBalancer(lb *network.LoadBalancer, existingLBs []network.LoadBalancer, service *v1.Service, clusterName string) error {
	...
	if v4Enabled {
		lbBackendPoolIDs = append(lbBackendPoolIDs, az.getBackendPoolID(lbName, lbResourceGroup, getBackendPoolName(clusterName, false)))
	}
	if v6Enabled {
		lbBackendPoolIDs = append(lbBackendPoolIDs, az.getBackendPoolID(lbName, lbResourceGroup, getBackendPoolName(clusterName, true)))
	}
	if isBackendPoolPreConfigured {
		...
		if deleteErr := az.safeDeleteLoadBalancer(*lb, clusterName, vmSetName, service); deleteErr != nil {
			...
			if err := az.VMSet.EnsureBackendPoolDeletedFromVMSets(vmssNamesMap, lbBackendPoolIDs); err != nil {
...
func (az *Cloud) safeDeleteLoadBalancer(lb network.LoadBalancer, clusterName, vmSetName string, service *v1.Service) *retry.Error {
	...
	v4Enabled, v6Enabled := getIPFamiliesEnabled(service)
	if v4Enabled {
		lbBackendPoolID := az.getBackendPoolID(pointer.StringDeref(lb.Name, ""), az.getLoadBalancerResourceGroup(), getBackendPoolName(clusterName, false))
		lbBackendPoolIDs = append(lbBackendPoolIDs, lbBackendPoolID)
	}
	if v6Enabled {
		lbBackendPoolID := az.getBackendPoolID(pointer.StringDeref(lb.Name, ""), az.getLoadBalancerResourceGroup(), getBackendPoolName(clusterName, true))
		lbBackendPoolIDs = append(lbBackendPoolIDs, lbBackendPoolID)
	}
	if _, err := az.VMSet.EnsureBackendPoolDeleted(service, lbBackendPoolIDs, vmSetName, lb.BackendAddressPools, true); err != nil {
...
func (az *Cloud) isFrontendIPChanged(clusterName string, config network.FrontendIPConfiguration, service *v1.Service, defaultLBFrontendIPConfigName string, pips *[]network.PublicIPAddress) (bool, error) {
	isInternal := requiresInternalLoadBalancer(service)
	isIPv6, err := az.isFIPIPv6(&config, pips, isInternal)
	if err != nil {
		return false, err
	}
...
func (az *Cloud) reconcileLoadBalancer(clusterName string, service *v1.Service, nodes []*v1.Node, wantLb bool) (*network.LoadBalancer, error) {
	...
	for _, ownedFIPConfigSingleStack := range ownedFIPConfigs {
		...
		isIPv6, err := az.isFIPIPv6(ownedFIPConfigSingleStack, &pips, isInternal)
		if err != nil {
			return nil, err
		}
	...
	if !wantLb {
		...
		if len(ownedFIPConfigs) < getExpectedOwnedConfigsCount(v4Enabled, v6Enabled) {
			...
			for _, ownedFIPConfig := range ownedFIPConfigs {
				isIPv6, err := az.isFIPIPv6(ownedFIPConfig, pips, isInternal)
				if err != nil {
					return nil, toDeleteConfigs, false, err
				}
				needs[isIPv6] = false
