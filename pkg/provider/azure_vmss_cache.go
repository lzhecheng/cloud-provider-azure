...
func (ss *ScaleSet) gcVMSSVMCache() error {
	vmssCached, err := ss.vmssCache.Get(consts.VMSSKey, azcache.CacheReadTypeUnsafe)
	if err != nil {
		return err
	}

	vmsses := vmssCached.(*sync.Map)
	removed := map[string]bool{}
	ss.vmssVMCache.Range(func(key, value interface{}) bool {
		cacheKey := key.(string)
		vlistIdx := cacheKey[strings.LastIndex(cacheKey, "/")+1:]
...
func (ss *ScaleSet) DeleteCacheForNode(nodeName string) error {
	...
	// GetWithoutCopy() is used because Get() returns deep copied object so cache cannot be deleted in that case.
	vmcache, err := timedcache.GetWithoutCopy(cacheKey, azcache.CacheReadTypeUnsafe)
	if err != nil {
		klog.Errorf("DeleteCacheForNode(%s) failed with error: %v", nodeName, err)
		return err
	}
